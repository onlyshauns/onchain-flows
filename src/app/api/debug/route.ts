...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    if (apiKey) {
      logs.push(`2. API Key found: ${apiKey.substring(0, 4)}...${apiKey.substring(apiKey.length - 4)}`);
    } else {
      logs.push('2. ⚠️ No ETHERSCAN_API_KEY found in environment');
    }

    const client = getEtherscanClient();
    logs.push('3. Client created successfully');

    // Test single address first
    logs.push('4. Testing single Binance address...');
    const binanceTxs = await client.getTokenTransfers('0x28C6c06298d514Db089934071355E5743bf21d60', {
      offset: 10,
      sort: 'desc',
    });
    logs.push(`5. Got ${binanceTxs.length} transactions from Binance address`);
    ...
    if (binanceTxs.length > 0) {
      ...
      logs.push(`6. Most recent tx: ${recent.hash.substring(0, 20)}...`);
      ...
      logs.push('6. ⚠️ No transactions returned from Etherscan API');
    }

    // Now test the whale movements function
    logs.push('7. Testing getRecentWhaleMovements (24h filter)...');
    const transactions = await client.getRecentWhaleMovements();
    logs.push(`8. Got ${transactions.length} transactions after 24h filter`);

    const oneDayAgo = Math.floor(Date.now() / 1000) - (24 * 60 * 60);
    logs.push(`9. 24h cutoff: ${new Date(oneDayAgo * 1000).toISOString()}`);
