...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    if (chainList.length === 0) {
      console.log('[API] No supported chains selected for Fund Movements DEX trades');
      return NextResponse.json({ flows: [], total: 0, source: 'None' });
    }

    console.log('[API] Fetching DEX trades for fund movements on chains:', chainList);

    try {
      const response = await client.getDEXTrades(chainList, {
        minValueUsd: 500000, // $500k+ for institutional size
        limit: 100,
      });
      ...
      if (response.data && response.data.length > 0) {
        dataSource = 'Nansen (Large DEX Trades $500K+)';
