...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    if (chainList.length === 0) {
      console.log('[API] No supported chains selected for Public Figures DEX trades');
      return NextResponse.json({ flows: [], total: 0, source: 'None' });
    }

    console.log('[API] Fetching DEX trades for public figures on chains:', chainList);

    try {
      const response = await client.getDEXTrades(chainList, {
        minValueUsd: 50000, // $50k+ for notable activity
        limit: 100,
      });
      ...
      if (response.data && response.data.length > 0) {
        dataSource = 'Nansen (Notable DEX Trades $50K+)';
        ...
        response.data.forEach((trade) => {
          ...
          // Filter out same-entity transfers (like exchange internal moves)
          if (isSameEntityTransfer(traderLabel, trade.dex_name || 'DEX')) {
            return;
          }
          ...
          allFlows.push({
            ...
            metadata: {
              category: 'Notable Trading Activity',
            },
          });
        });
      }
